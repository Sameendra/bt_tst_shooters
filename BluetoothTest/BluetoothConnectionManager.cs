...
namespace Shooters.Bluetooth
{
    public class BluetoothConnectionManager : PropertyChangedBase
    {
        ...
        public async Task EnumerateDevicesAsync(ObservableCollection<DeviceInformation> deviceList)
        {
            this.State = BluetoothConnectionState.Enumerating;
            DeviceInformationCollection devInfoColl = await DeviceInformation.FindAllAsync(RfcommDeviceService.GetDeviceSelector(RfcommServiceId.SerialPort));

            foreach (var devInfo in devInfoColl)
            {
                deviceList.Add((DeviceInformation)devInfo);
            }
